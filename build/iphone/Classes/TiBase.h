...
// in simulator we redefine to format for PicPocket Developer console
...
{\
	NSString * exceptionName = [@"org.picpocket." stringByAppendingString:NSStringFromClass([self class])];\
...
+ (void) throwException:(NSString *) reason subreason:(NSString*)subreason location:(NSString *)location\
{\
	NSString * exceptionName = @"org.picpocket";\
	NSString * message = [NSString stringWithFormat:@"%@. %@ %@",reason,(subreason!=nil?subreason:@""),(location!=nil?location:@"")];\
	NSLog(@"[ERROR] %@",message);\
	if ([NSThread isMainThread]==NO) {\
		@throw [NSException exceptionWithName:exceptionName reason:message userInfo:nil];\
...
enum {
	UIPicPocketNativeItemNone = -1, 
	UIPicPocketNativeItemSpinner = -2,
	UIPicPocketNativeItemProgressBar = -3,
	
	UIPicPocketNativeItemSlider = -4,
	UIPicPocketNativeItemSwitch = -5,
	UIPicPocketNativeItemMultiButton = -6,
	UIPicPocketNativeItemSegmented = -7,
	
	UIPicPocketNativeItemTextView = -8,
	UIPicPocketNativeItemTextField = -9,
	UIPicPocketNativeItemSearchBar = -10,
	
	UIPicPocketNativeItemPicker = -11,
	UIPicPocketNativeItemDatePicker = -12,
	
	UIPicPocketNativeItemInfoLight = -13,
	UIPicPocketNativeItemInfoDark = -14,
	
	UIPicPocketNativeItemDisclosure = -15,
	
	UIPicPocketNativeItemContactAdd = -16
};
